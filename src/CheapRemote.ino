...
bool tryToConnect() {
  ...
  if (wifiInitConnection()) {
    ...
    M5.Lcd.print("Connesso:" + WiFi.localIP().toString());
...
void setup() {
  ...
  WiFi.mode(WIFI_STA);
  Serial.println("[SETUP] Disconnected from network\n");
  wifiDisconnect();
  delay(1000);
}
...
void loop() {
  ...
  if (wifiCheckConnection()) {
    ...
    if (goProJson != "") {
      ...
      M5.Lcd.setTextColor(TFT_WHITE);
      M5.Lcd.print("Batt: " + batteryStatus + " (" + String(batteryPercent) +  "%)");
      ...
      M5.Lcd.setCursor(0, 20);
      M5.Lcd.setTextColor(TFT_RED);
      M5.Lcd.print("GoPro not respondig...");
